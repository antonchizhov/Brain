...
constructor TMilescuNa.Create(nrn: TNeuron);
begin  inherited Create; oN:=nrn; end;

{-------- Na - Markov model -----------------------}
function TMilescuNa.TransitionRate(v2,k,V12 :double) :double;
...
procedure TMilescuNa.alphas_Na_Milescu(v2 :double; var A :arr26x26);
var
    i,j                                   :integer;
...
begin
  ...
  if (oN.NP.Na_subtype='Milescu-13') or (oN.NP.Na_subtype='Milescu-26') then begin
      a_:=0.71098;  a2:=a_*a_;
      b_:=8.1799;   b2:=b_*b_;
  ...
  end else if oN.NP.Na_subtype='Carter-12' then begin
      a_:=2.51;   a2:=a_*a_;
      b_:=5.32;   b2:=b_*b_;
  ...
  A[ 6,12]:=  beho;    // O6 -> I12
  A[12, 6]:=  alho;    // I12 -> O6
  if oN.NP.Na_subtype='Milescu-13' then begin
     al:=2.073e-4 *TransitionRate(v2,-0.03096,0);
     be:=0.4008   *TransitionRate(v2,-0.01101,0);
     A[ 6,13]:=  be;    // O6 -> I13
     A[13, 6]:=  al;    // I13 -> O6
  end else if oN.NP.Na_subtype='Milescu-26' then begin
  ...
  {*************************************** 26 states **************************}
  if oN.NP.Na_subtype='Milescu-26' then begin
...
procedure TMilescuNa.tau_inf_SlowInactivation(v2:double; var tau_i,i_inf:double);
var v3,a,b :double;
begin
...
function TMilescuNa.gNa_inf_Milescu:double;
...
var
     ...
     i,j        :integer;
begin
  alphas_Na_Milescu(oN.NV.V*1000, A);
  ...
  oN.NV.mmR:=oN.NV.XNa[6];
  if NM>13 then oN.NV.mmR:=oN.NV.mmR+oN.NV.XNa[19];
  oN.NV.hhR:=1;
  gNa_inf_Milescu:=oN.NP.gNa*oN.NV.mmR*(1-oN.NP.IfBlockNa);
end;

function TMilescuNa.Conductance(x,y,z :double):double;
begin
  Conductance:=x*z*z*z*(1-oN.NP.IfBlockNa);      // mh^3 [Fernandez, White 2010]
end;

function TMilescuNa.Current:double;
var
     A                      :arr26x26;
...
begin
  ...
  for i:=1 to NM-1 do begin
      ...
      X[i]:=oN.NV.XNa[i] + E_exp(dt,1/Sm)*(Sp/Sm - oN.NV.XNa[i]);
  end;
  for i:=1 to NM-1 do  oN.NV.XNa[i]:=X[i];
  { Normalization }
  S:=0;  for i:=1 to NM-1 do S:=S + oN.NV.XNa[i];
  oN.NV.XNa[NM]:=max(1-S,0);
  oN.NV.mmR:=oN.NV.XNa[6];
  if NM>13 then oN.NV.mmR:=oN.NV.mmR+oN.NV.XNa[19];
  oN.NV.hhR:=1;
  { Slow inactivation }
  ...
  Current:=oN.NP.gNa*Conductance(oN.NV.mmR,0,oN.NV.iiR)*(oN.NV.V-oN.NP.VNa);
end;

procedure TMilescuNa.Init;
var   dum,tau_i :double;
begin
  NM:=26;
  if oN.NP.Na_subtype='Milescu-13' then NM:=13;
  if oN.NP.Na_subtype='Milescu-26' then NM:=26;
  if oN.NP.Na_subtype='Carter-12'  then NM:=12;
  dum:= gNa_inf_Milescu;
  { Slow inactivation }
  tau_inf_SlowInactivation(oN.NV.V*1000, tau_i,oN.NV.iiR);
  if (oN.NP.Na_subtype='Carter-12')and(dt>0.02/1000) then dt:=0.02/1000; {s}
end;
