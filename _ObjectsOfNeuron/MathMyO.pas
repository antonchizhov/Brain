...
procedure New_m4(dt_tau,T :double; var m,U :double);
procedure New_m5(tau,dt,T :double; var m :double);
function Erfic(x :double):double;
function InvErfic(x :double):double;
...
function LogNorm_PartialExpectation(k,mu,sgm :double) :double;
function LogNormLinearNormalizedDistribution(k,mu,sgm :double) :double;
function fNMDA(V,Mg :double) :double;
...
function E_exp(dt,tau :double):double;
begin
  if tau>0 then E_exp:= 1 - dexp(-dt*1000/tau) else E_exp:=1;
end;
...
procedure New_m4(dt_tau,T :double; var m,U :double);
{
 Step of eq.  ddm/ddt + 2/tau dm/dt + 1/tau^2 m = 1/tau^2 T
 Maximum here depends on tau, but the charge does not!}
begin
  if dt_tau>=1 then
     m:=T
  else begin
...
begin
  a:=0.140012;
  x2:=x*x;
  Erfic:=sgn(x)*sqrt(1-dexp(-x2*(4/pi+a*x2)/(1+a*x2)));
end;

function InvErfic(x :double):double;
var x2,q,a :double;
begin
   if abs(x)>0.999999 then x:=sgn(x)*0.999999;
   a:=0.140012;
   x2:=x*x;
   q:=2/pi/a+ln(1-x2)/2;
   InvErfic:=sgn(x)*sqrt(sqrt(sqr(q) - ln(1-x2)/a)- q);
end;
...
function LogNorm_PartialExpectation(k,mu,sgm :double) :double;
{ Wiki: g(k) = \int_k^\infty x*ln(N(x))*dx
             = exp(mu+1/2*sgm^2) * F((mu+sgm^2-ln(k))/sgm),
  where F((ln(x')-mu')/sgm) = LogNormF(x',mu',sgm),
        ln(N(x))            = LogNorm (x,mu,sgm).
  Note that:  mu+sgm^2-ln(k)=ln(exp(mu+sgm^2)/k) }
var z :double;
begin
  if k>0 then z:=dexp(mu+1/2*sgm*sgm) * LogNormF(dexp(mu+sgm*sgm)/k,0,sgm)
         else z:=dexp(mu+1/2*sgm*sgm);
  LogNorm_PartialExpectation:=z;
end;

function LogNormLinearNormalizedDistribution(k,mu,sgm :double) :double;
{ y(k)=\int_0^k x*ln(N(x))*dx / \int_0^\infty x*ln(N(x))*dx }
var z0 :double;
begin
  z0:=LogNorm_PartialExpectation(0,mu,sgm);
  LogNormLinearNormalizedDistribution:=1-LogNorm_PartialExpectation(k,mu,sgm)/z0;
end;

function fNMDA(V,Mg :double) :double;
begin
  fNMDA:=1/(1+Mg/3.57*dexp(-62*V));
end;
