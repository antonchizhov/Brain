...
type
  ...
  public
    ...
    {Variables:}
    Bum,xLN                                     :array[1..MaxLN] of double;
    ...
    function ro_at_ts(ts_ :double; iLN :integer) :double;
...
function TPopulation.ro_at_ts(ts_ :double; iLN :integer) :double;
var
    i,i2,nts    :integer;
    t1,t2,R1,R2 :double;
begin
...
begin
  ...
  mean_U:=0;
  for iLN:=1 to MaxLN do begin
      mean_U:=mean_U + VPeak_*Bum[iLN] + U*ro[PP.Nts,iLN]*PP.dts;
      for i:=0 to PP.Nts-1 do begin
          V_:=Nrn[i].NV.V;
          ...
          mean_U:=mean_U + V_*ro[i,iLN]*min(PP.dts,PP.ts_end-ts[i]);
      end;
  end;
...
begin
  ...
  for i:=0 to PP.Nts do begin
      ...
      if PP.IfLogNorm=1 then begin { If lognormal }
         Ufr_:=Ufree_ts(ts[i]);
...
begin
  ...
  { Fulfil density }
  for iLN:=1 to MaxLN do begin
      for i:=0 to PP.Nts do  ro[i,iLN]:=0;   ro[PP.Nts,iLN]:=1/PP.dts;
      for i:=0 to PP.Nts do  LogN.ro_0[i,iLN]:=ro[i,iLN];
  end;
  ...
  for i:=0 to PP.Nts-1 do begin
      if ts[i]>=PP.ts_end then begin
         ...
         for iLN:=1 to MaxLN do begin
             ...
             ro[i,iLN]:=Bum[iLN]/(t-tBum);
             Bum[iLN]:=0;
             LogN.ro_0[i,iLN]:=ro[i,iLN];  {number of neurons attributed to a particle}     {LN}
         end;
...
var
    i,iLN                                                              :integer;
    dro,S,H_dt,F,sgmV_,Vsc_,dVdt,A,g0_,g_g0,taum_,H,V_,l_,PHI,Ufr_,dx_ :double;
begin
...
{}      VT[i]:=VThreshold3(PP.NP,dVdt,ts[i]);  {changed from 5 to 3 on 20.06.2011}
  ...
  nu:=0;
  FOR iLN:=1 to MaxLN DO BEGIN
      PHI:=LogNorm(xLN[iLN],LogN.mu,LogN.sgm);
      if iLN=1 then dx_:=xLN[1] else dx_:=xLN[iLN]-xLN[iLN-1];
      if PP.IfLogNorm=0 then begin PHI:=1; dx_:=1; end;
      S:=0;
      FOR i:=PP.Nts downto 0 do begin                            { loop for t* }
          ...
          Vsc_:=(V_-VT[i])/sgmV_/sqrt(2)*g_g0;
          ...
          S:=S         + dro;
      END;
      nu:=nu + S*PP.dts/dt*PHI*dx_;
      Bum[iLN]:=Bum[iLN]+S*PP.dts;
  END;
  { Norm }
  S:=0;
  for iLN:=1 to MaxLN do begin
      S:=S + Bum[iLN] + ro[PP.Nts,iLN]*PP.dts;
      for i:=0 to PP.Nts-1 do  S:=S+ro[i,iLN]*min(PP.dts,PP.ts_end-ts[i]);
  end;
  Norm:=S;
  {***}
  nu:=nu/Norm;
